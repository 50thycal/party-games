...
export default function SubwayHotseat() {
  ...
  return <main className="min-h-screen bg-[#10232d] p-2 text-white sm:p-3" style={{fontFamily:"ui-sans-serif, system-ui, sans-serif"}}>
    ...
    {help && <aside className="mb-2 rounded-xl bg-[#f5f3e9] p-4 text-sm leading-relaxed text-slate-900"><b>Your first journey</b><ol className="ml-5 mt-2 list-decimal space-y-1"><li>Draft one line at list price each turn. Everyone ends with three lines; there is no pass.</li><li>Receive one two-neighborhood and one three-neighborhood Destination at start. Draft three Engineering goals from two face-up choices or a blind draw.</li><li>Each round, hire crews for different lines: $1M / $3M / $6M for 1 / 2 / 3 crews. {crewActivationText(session.game.bendMode)} Start with $40M; construction lasts at most nine rounds. Buy one extra Destination for $5M before hiring, once per game.</li><li>Place starter pegs on empty border holes; placement is free unless you join an opponent’s station. Follow each line’s printed segment lengths; turns may be up to 90°.</li><li>Tap a glowing peg, inspect the next step and toll, then Confirm. You can plan ahead without committing.</li><li>Score finished lines, connected missions and Engineering goals. Longest Network earns 5 VP (3 each if tied). Largest Transfer Station awards 6 VP to its leading company, 3 each to two tied leaders, 2 each to three, and none to four. Keep debt low. Most points wins!</li></ol><p className="mt-2">Use Board, Schedule, Lines and Cards to move around the table. Drag to pan; scroll or pinch to zoom. Pass the device before revealing the next company’s cards. Different lines of your company transfer at horizontally/vertically adjacent stations; one peg per hole. Joining an opponent station costs $1M once for that line at that station. Every new crossing still costs $1M. North is top, south bottom, west left and east right.</p></aside>}
