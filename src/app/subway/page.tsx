...
const SAVE_KEY = "subway-hotseat-v10";
...
export default function SubwayHotseat() {
  ...
  return <main className="min-h-screen bg-[#10232d] p-2 text-white sm:p-3" style={{fontFamily:"ui-sans-serif, system-ui, sans-serif"}}>
    <header className="mb-2 flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-[#18343f] p-2">
      ...
      <label className="text-xs text-slate-300">Playing as <select aria-label="Active company" value={session.seat} onChange={(e)=>commit({...session, seat:e.target.value})} className="ml-1 rounded-lg bg-[#10232d] p-2 text-sm font-bold text-white">{session.room.players.map((p)=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      ...
      <button onClick={()=>setHelp(!help)} aria-expanded={help} className="rounded-lg bg-white/10 px-3 py-2 text-xs">How to play</button>
