...
export function PlayerPads({game,roomKey}:{game:SubwayState;roomKey:string}) {
  ...
  return <section aria-label="Player panels" className="pointer-events-auto relative w-full rounded-xl border border-teal-700 bg-[#10252e] p-2 text-white">
    ...
    <div role="status" aria-live="polite" className={`${event?'min-h-5':'sr-only'} text-center text-xs text-amber-200`}>{event?`${event.reversed?'Undo · ':''}${event.payments.map(p=>p.from==='bank'?`${game.players[p.to!]?.name} ${event.reversed?'−':'+'}$${p.amount}M · ${event.reversed?'completion reversed':'line complete'}`:`${game.players[p.from]?.name} ${event.reversed?'receives refund from':'pays'} ${game.players[p.to!]?.name??'the bank'} $${p.amount}M · ${p.reason}`).join(' · ')}`:'Company lines and cash'}</div>
