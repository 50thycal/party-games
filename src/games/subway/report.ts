...
export function generateAiPlaytestReport(game: SubwayState, context: SubwayReportContext = {}): string {
  ...
  const lines: string[] = [
    ...
    JSON.stringify({
      ...
      debtVpPerMillion: SUBWAY_CONFIG.contact.debtVpPerMillion,
      ...
      bendRules: 'Tokens: 3 per company, extras $3M cash; delayed: one leg per activation; maximum one bend per segment; max 90-degree curve; total path length; bends/worksites are not pegs; partial track excludes network length until finished',
      stationAccess: "Free station connections, including starters; crossing payments remain $1M",
