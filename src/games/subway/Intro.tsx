...
type Options = { bendMode?: BendMode };
type Slide = { title: string; kicker: string; text: string; note: string; art: string; tiles?: string[] };
export function introSlides({ bendMode = "delayed" }: Options): Slide[] {
  return [
    ...
    {kicker:"Draft your plan",title:"Read the recipe left to right.",text:"Buy three Line Contracts, one at a time. Each number is the next segment’s length in peg spaces.",note:"Draft three Engineering goals; you are dealt two Destinations, one pair and one triple. Engineering scope matters: Single Line, Connected Network or Company-wide. Goals stay in hand; nothing to lock in.",art:"recipe",tiles:["Example · Pink Line","2 + 3 + 4 + 2 = 11 spaces"]},
    ...
    {kicker:"Read the distance",title:"Build the printed length.",text:"Every segment must match its printed length, within ±0.5 peg space. Measure it like a tight string from peg to peg: one diagonal step is about 1.4 spaces.",note:"Measure the whole path, including both legs of a bend. Recipe totals always sum the printed numbers.",art:"length"},
    ...
    {kicker:"After all three lines finish",title:"Keep your network growing.",text:"Starting on your next turn, extend any one of your lines 1–2 peg spaces for $1M plus crossing tolls, or skip for free.",note:"One straight placement, with normal heading and occupancy rules. Pay on Confirm; borrowing and Undo apply. No extra crew bill or repeated completion reward. Buy optional cards before selecting the extension.",art:"tiles",tiles:["One extension per turn","Original recipes stay complete","New stations serve your goals"]},
    {kicker:"Ready to play",title:"Make the connections count.",text:"Check your goals. Choose your crews. Preview the route. Confirm when you’re ready.",note:"Play ends after round 9, or earlier if no company has a legal build or extension. Score lines, goals, awards and ending cash. Reopen How to play whenever you need it.",art:"network",tiles:["Stations make connections","Neighborhoods are places","Crossings are not transfers"]},
  ];
}
