...
for (const count of [2,3,4]) {
  ...
  assert.deepEqual(prepared.preview,{x:route[0].x,y:route[0].y});
  ...
  assert.equal(preparePlan(built,id,li,saved).preview?.x,route[1].x);
  ...
  assert.equal(preparePlan(divergent,id,li,saved).preview,null,"stale plans cannot choose a real target");
